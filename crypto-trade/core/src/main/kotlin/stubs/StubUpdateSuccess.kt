...
fun CorChainDsl<OrderContext>.stubUpdateSuccess(title: String, corSettings: CorSettings) = worker {
    this.title = title
    this.description = """
        Кейс успеха для изменения заявки
    """.trimIndent()
    ...
    handle {
        logger.doWithLogging(id = this.requestId.asString(), LogLevel.DEBUG) {
            ...
            val stub = OrderStub.prepareResult {
                orderRequest.id.takeIf { it != OrderId.NONE }?.also { this.id = it }
